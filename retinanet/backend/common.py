...
def bbox_transform_inv(boxes, deltas, mean=None, std=None):
    ...
    @param boxes  : np.array of shape (B, N, 4), where B is the batch size, N the
                    number of boxes and 4 values for (x1, y1, x2, y2).
    @param deltas : np.array of same shape as boxes. These deltas (d_x1, d_y1,
                 d_x2, d_y2) are a factor of the width/height.
    @param mean   : The mean value used when computing deltas (defaults to
                    [0, 0, 0, 0]).
    @param std    : The standard deviation used when computing deltas
                    (defaults to [0.2, 0.2, 0.2, 0.2]).

    @returns
        A np.array of the same shape as boxes, but with deltas applied to each
        box. The mean and std are used during training to normalize the
