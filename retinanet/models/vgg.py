...
def vgg_retinanet(num_classes, backbone='vgg16', inputs=None, modifier=None, **kwargs):
    ...
    @param backbone    : Which backbone to use (one of ('vgg16', 'vgg19')).
    @param inputs      : The inputs to the network (defaults to a Tensor of shape
                         (None, None, 3)).
    @param modifier    : A function handler which can modify the backbone before
                         using it in retinanet (this can be used to freeze backbone
                         layers for example).
