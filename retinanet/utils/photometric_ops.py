...
class ConvertColor:
    ...
    def __init__(self, current='RGB', to='HSV', keep_3ch=True):
        """!@brief
        @param current (str, optional)   : The current color space of the images.
                                           Can be one of 'RGB' and 'HSV'.
        @param to (str, optional)        : The target color space of the images.
                                           Can be one of 'RGB', 'HSV', and 'GRAY'.
        @param keep_3ch (bool, optional) : Only relevant if `to == GRAY`.
                                           If `True`, the resulting grayscale
                                           images will have three channels.
        """
...
class ConvertDataType:
    ...
    def __init__(self, to='uint8'):
        """!@brief
        @param to (string, optional): To which datatype to convert the input images.
...
class Hue:
    ...
    def __init__(self, delta):
        """!@brief
        @param delta (int): An integer in the closed interval `[-180, 180]` that
                            determines the hue change, where a change by integer
                            `delta` means a change by `2 * delta` degrees. Read
                            up on the HSV color format if you need more information.
        """
...
class RandomHue:
    ...
    def __init__(self, max_delta=18, prob=0.5):
        """!@brief
        @param max_delta (int)        : An integer in the closed interval
                                        `[0, 180]` that determines the maximal
                                        absolute hue change.
        @param prob (float, optional) : `(1 - prob)` determines the probability
                                        with which the original, unaltered image
                                        is returned.
        """
...
class Saturation:
    ...
    def __init__(self, factor):
        """!@brief
        @param factor (float): A float greater than zero that determines
                               saturation change, where values less than one
                               result in less saturation and values greater than
                               one result in more saturation.
        """
...
class RandomSaturation:
    ...
    def __init__(self, lower=0.3, upper=2.0, prob=0.5):
        """!@brief
        @param lower (float, optional) : A float greater than zero, the lower
                                         bound for the random saturation change.
        @param upper (float, optional) : A float greater than zero, the upper
                                         bound for the random saturation change.
...
class Brightness:
    ...
    def __init__(self, delta):
        """!@brief
        @param delta (int): An integer, the amount to add to or subtract from
...
class RandomBrightness:
    ...
    def __init__(self, lower=-84, upper=84, prob=0.5):
        """!@brief
        @param lower (int, optional)  : An integer, the lower bound for the
                                        random brightness change.
        @param upper (int, optional)  : An integer, the upper bound for the
                                        random brightness change.
                                        Must be greater than `lower`.
        @param prob (float, optional) : `(1 - prob)` determines the probability
                                        with which the original, unaltered image
...
class Contrast:
    ...
    def __init__(self, factor):
        """!@brief
        @param factor (float): A float greater than zero that determines contrast
                               change, where values less than one result in less
                               contrast and values greater than one result in more
                               contrast.
        """
        if factor <= 0.0: raise ValueError("It must be `factor > 0`.")
        self.factor = factor
...
class RandomContrast:
    ...
    def __init__(self, lower=0.5, upper=1.5, prob=0.5):
        """!@brief
        @param lower (float, optional) : A float greater than zero, the lower
                                         bound for the random contrast change.
        @param upper (float, optional) : A float greater than zero, the upper
                                         bound for the random contrast change.
                                         Must be greater than `lower`.
        @param prob (float, optional)  : `(1 - prob)` determines the probability
...
class Gamma:
    ...
    def __init__(self, gamma):
        """!@brief
        @param gamma (float): A float greater than zero that determines gamma change.
        """
...
class RandomGamma:
    ...
    def __init__(self, lower=0.25, upper=2.0, prob=0.5):
        """!@brief
        @param lower (float, optional) : A float greater than zero, the lower
                                         bound for the random gamma change.
        @param upper (float, optional) : A float greater than zero, the upper
                                         bound for the random gamma change.
                                         Must be greater than `lower`.
        @param prob (float, optional)  : `(1 - prob)` determines the probability
...
class ChannelSwap:
    ...
    def __init__(self, order):
        """!@brief
        @param order (tuple): A tuple of integers that defines the desired
                              channel order of the input images after the channel swap.
        """
        self.order = order
