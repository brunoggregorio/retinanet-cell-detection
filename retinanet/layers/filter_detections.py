...
def filter_detections(
    ...
    @param boxes                 : Tensor of shape (num_boxes, 4) containing the
                                   boxes in (x1, y1, x2, y2) format.
    @param classification        : Tensor of shape (num_boxes, num_classes)
                                   containing the classification scores.
    @param other                 : List of tensors of shape (num_boxes, ...) to
                                   filter along with the boxes and classification
                                   scores.
    @param class_specific_filter : Whether to perform filtering per class, or take
                                   the best scoring class and filter those.
    @param nms                   : Flag to enable/disable non maximum suppression.
    @param score_threshold       : Threshold used to prefilter the boxes with.
    @param max_detections        : Maximum number of detections to keep.
    @param nms_threshold         : Threshold for the IoU value to determine when
...
class FilterDetections(keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        @param nms                   : Flag to enable/disable NMS.
        @param class_specific_filter : Whether to perform filtering per class, or take the best scoring class and filter those.
        @param nms_threshold         : Threshold for the IoU value to determine when a box should be suppressed.
        @param score_threshold       : Threshold used to prefilter the boxes with.
        @param max_detections        : Maximum number of detections to keep.
        @param parallel_iterations   : Number of batch items to process in parallel.
        """
        self.nms                   = nms
    ...
    def call(self, inputs, **kwargs):
        ...
        @param inputs : List of [boxes, classification, other[0], other[1], ...] tensors.
        """
        boxes          = inputs[0]
